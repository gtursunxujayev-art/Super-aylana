...
export async function POST(req: Request) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!prize || !prize.active) {
      return NextResponse.json({ ok: false, error: 'PRIZE_INACTIVE' }, { status: 400 })
    }
    if (user.balance < prize.coinCost) {
    ...
    await tx.win.create({
      data: { userId: uid, prizeId: prize.id, title: prize.title, imageUrl: prize.imageUrl ?? null },
    })
